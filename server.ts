...
7. LANGUAGE: You MUST respond strictly in ${langName}. Even if they ask questions in another language, translate your feedback to ${langName} and proceed. Ensure natural, fluent, and caring language.`;
  ...
  // Stripe Webhook handler (Real-world production requirement)
  app.post("/api/webhook", express.raw({ type: "application/json" }), async (req, res) => {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (userId) {
        ...
        // In a real app, you'd use Admin SDK to update Firestore here:
        // await admin.firestore().collection('users').doc(userId).update({ plan: 'premium' });
      }
    }
